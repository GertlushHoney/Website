...
| Apiary environment | **Have it, partially** — Bramble Farm photos show a community growing site, not hives specifically; no photo of actual hives/frames/bees yet. `Bramble Farm  views 1.jpg` shows identifiable people at a community event — do not publish it without explicit consent from those pictured. `Bramble Farm 2.webp` (landscape only, no people) is used instead, e.g. on `/shop/experiences` for the Bee Day Experience card (moved there 2026-08-10 from the removed `/gifts` page) and as that category's shop-tile image |
